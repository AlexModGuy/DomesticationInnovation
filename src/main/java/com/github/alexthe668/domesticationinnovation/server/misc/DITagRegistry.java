...
public class DITagRegistry {
    ...
    private static TagKey<EntityType<?>> registerEntity(String name) {
        return TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation(DomesticationMod.MODID, name));
    }

    private static TagKey<Item> registerItem(String name) {
        return TagKey.create(Registries.ITEM, new ResourceLocation(DomesticationMod.MODID, name));
    }
}